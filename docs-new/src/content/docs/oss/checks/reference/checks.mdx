...
check = FnCheck(
    fn=lambda trace: len(trace.last.outputs) > 10,
...
`FnCheck` is perfect for quick prototyping and simple validation logic. For
...
check = LLMJudge(
    prompt_path="checks::safety.j2", generator=Generator(model="openai/gpt-5")
)
```
...
| Parameter            | Type                    | Default                                | Description                        |
| -------------------- | ----------------------- | -------------------------------------- | ---------------------------------- |
| `reference_text`     | `str \| None`           | `None`                                 | Reference text to compare against  |
| `reference_text_key` | `str`                   | `"trace.last.metadata.reference_text"` | JSONPath to extract reference text |
| `actual_answer_key`  | `str`                   | `"trace.last.outputs"`                 | JSONPath to extract actual value   |
...
generator = Generator(model="openai/gpt-5", temperature=0.1)
...
- [Custom Checks](/oss/checks/how-to/custom-checks/) - Build domain-specific
  checks with FnCheck, Check subclasses, and more
