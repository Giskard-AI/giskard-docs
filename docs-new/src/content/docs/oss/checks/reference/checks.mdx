...
`FnCheck` is perfect for quick prototyping and simple validation logic. For complex checks, consider creating a custom Check subclass.
...
The `context` parameter can be: a single string document, a list of string documents, or extracted from trace metadata using `context_key`.
...
In Jinja2 templates, prefer `trace.last` over `trace.interactions[-1]` for better readability. Both provide access to the most recent interaction.
...
Exactly one of `prompt` or `prompt_path` must be provided. Providing both or neither will raise a validation error.
...
- [Custom Checks](/oss/checks/how-to/custom-checks/) - Build domain-specific checks with FnCheck, Check subclasses, and more
