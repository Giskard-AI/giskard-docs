...
Base class for all checks. Subclass and register to create custom validation
logic.
...
The `Check` class provides the foundation for all validation logic in Giskard.
Create custom checks by subclassing and registering with a unique kind
identifier using the `@Check.register("kind")` decorator.
...
- `trace` (`Trace`): The trace containing interaction history. Access the
  current interaction via `trace.last` or `trace.interactions[-1]`

**Returns:**

- `CheckResult`: Success, failure, error, or skip result

:::note The `run` method may be async. Use `await check.run(trace)` when calling
asynchronously. :::
...
:::tip[Best Practice] Use the `@Check.register()` decorator to make your check
discoverable and enable polymorphic serialization. This allows your checks to be
saved, loaded, and shared across your testing suite. :::
...
`CheckResult` encapsulates the outcome of a check execution, including the
status (pass/fail/error/skip), optional message, metrics, and additional
details.
...
result = CheckResult.success(
    message="All validations passed", details={"score": 0.95, "threshold": 0.8}
)
...
result = CheckResult.error(
    message="Failed to connect to API", details={"error": "Connection timeout"}
)
...
An interaction represents one exchange in a conversation or workflow, capturing
the inputs provided, the outputs produced, and optional metadata.
...
:::note Interactions are typically created through the scenario builder's
`.interact()` method rather than directly instantiated. :::
...
execution. It is passed to checks for validation and to interaction specs for
generating subsequent interactions.

The trace is immutable (frozen), ensuring that checks and specs cannot
accidentally modify the history.
...
:::tip[Accessing Trace in Templates] The `trace.last` property is available in
Jinja2 prompt templates and JSONPath expressions, making it easy to reference
...
`InteractionSpec` defines how to generate interactions in a scenario. It can use
static values or callables that compute values based on the current trace.
...
# Static values
test_case = Scenario("static_example").interact(
    inputs="test input", outputs="test output"
)

# Callable outputs - dynamic generation
test_case = Scenario("dynamic_example").interact(
    inputs="test query", outputs=lambda inputs: my_model(inputs)
)

# Access trace context
test_case = Scenario("context_example").interact(
    inputs=lambda trace: (
        f"Previous: {trace.last.outputs if trace.last else 'None'}"
    ),
    outputs=lambda inputs: generate_response(inputs),
)
```

:::tip[Dynamic Interactions] Use callables for `inputs` and `outputs` when you
need to generate values based on the current trace context or previous
interactions. :::
...
A `Scenario` represents a multi-step test workflow combining interactions and
checks. Each interaction is executed in order, building up the trace, and checks
can be inserted at any point to validate the state.
...
test_scenario = (
    ...
    .check(
        FnCheck(fn=lambda trace: "Hi" in trace.last.outputs, name="greeting_check")
    )
    # Second interaction
    .interact(inputs="What's the weather?", outputs="It's sunny!")
    # Final validation
    .check(
        FnCheck(fn=lambda trace: len(trace.interactions) == 2, name="count_check")
    )
)
...
print(f"Interactions: {len(result.final_trace.interactions)}")
```
...
- `outputs`: Static value or callable `(inputs) -> value` or
  `(trace, inputs) -> value`
...
A `TestCase` groups multiple scenarios for batch execution. Each scenario runs
independently with its own trace.

:::note[Internal Implementation] `TestCase` is an internal implementation
detail. Users should always use the `Scenario()` class to create scenarios. The
TestCase provides the execution engine that runs scenarios. :::
...
test_scenario = (
    ...
    .check(FnCheck(fn=lambda trace: True, name="check1"))
    .check(FnCheck(fn=lambda trace: True, name="check2"))
)
...
### JSONPath extraction with `resolve`

Extract values using JSONPath expressions via `resolve` from
...
should use the higher-level `Scenario(...).run()` API rather than using
ScenarioRunner directly.

:::note This is a low-level API. Prefer using `Scenario(...).run()` for most use
cases. :::
...
- [Testing Utilities](/oss/checks/reference/utils/) - Testing utilities and
  helpers
