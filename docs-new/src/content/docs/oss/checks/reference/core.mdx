...
:::note The `run` method may be async. Use `await check.run(trace)` when calling asynchronously. :::
...
:::tip[Best Practice] Use the `@Check.register()` decorator to make your check discoverable and enable polymorphic serialization. This allows your checks to be saved, loaded, and shared across your testing suite. :::
...
:::note Interactions are typically created through the scenario builder's `.interact()` method rather than directly instantiated. :::
...
:::tip[Accessing Trace in Templates] The `trace.last` property is available in Jinja2 prompt templates and JSONPath expressions, making it easy to reference the most recent interaction. :::
...
:::tip[Dynamic Interactions] Use callables for `inputs` and `outputs` when you need to generate values based on the current trace context or previous interactions. :::
...
:::note[Internal Implementation] `TestCase` is an internal implementation detail. Users should always use the `Scenario()` class to create scenarios. The TestCase provides the execution engine that runs scenarios. :::
...
Extract values using JSONPath expressions via `resolve` from `giskard.checks.core.extraction`.
...
:::note This is a low-level API. Prefer using `Scenario(...).run()` for most use cases. :::
