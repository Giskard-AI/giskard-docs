...
`Scenario` provides a chainable interface for constructing multi-step test
scenarios. Each method returns the scenario (or self), allowing for method
chaining. There is no separate `ScenarioBuilder` class — `Scenario` is the
builder.
...
scenario.check(Equals(expected_value="success", key="trace.last.outputs.status"))
```
...
`Suite` holds multiple `Scenario` instances and runs them serially. Optionally
bind a shared target SUT at suite level or override it in `run()`.
