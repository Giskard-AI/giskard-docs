...
Helper functions and utilities for normalization, value providers, and parameter
injection.
...
Utilities for normalizing strings and data structures using Unicode
normalization forms.
...
:::tip[When to Use Normalization] Use normalization when comparing text that may
...
provider = CallableValueProvider(
    callable=compute_value, injection_mapping=mapping
)
...
:::note[Advanced Usage] Value providers are primarily used internally by the
framework. Most users won't need to interact with them directly. :::
...
injection = ParameterInjection(name="trace", source="context")
```

:::note[Framework Internal] Parameter injection is primarily used internally for
dependency injection in checks and interaction specs. Most users won't need to
use this directly. :::
...
:::tip[When to Use] Use `a_generator` when you need to handle different types of
inputs (values, sync generators, async generators) uniformly as async
generators. :::
...
class NormalizedComparison(Check):
    ...
    async def run(self, trace: Trace) -> CheckResult:
        ...
        expected_norm = normalize_string(
            self.expected, normalization_form=self.normalization_form
        )
        actual_norm = normalize_string(
            actual, normalization_form=self.normalization_form
        )
...
provider = DatabaseValueProvider(
    query="SELECT name FROM users WHERE id = :user_id", db_connection=db
)
