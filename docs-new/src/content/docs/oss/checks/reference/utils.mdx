...
:::tip[When to Use Normalization] Use normalization when comparing text that may have different Unicode representations:
...
:::note[Advanced Usage] Value providers are primarily used internally by the framework. Most users won't need to interact with them directly. :::
...
:::note[Framework Internal] Parameter injection is primarily used internally for dependency injection in checks and interaction specs. Most users won't need to use this directly. :::
...
:::tip[When to Use] Use `a_generator` when you need to handle different types of inputs (values, sync generators, async generators) uniformly as async generators. :::
