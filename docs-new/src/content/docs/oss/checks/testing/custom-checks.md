...
While Giskard Checks provides many built-in checks, you'll often need
domain-specific validation. This guide shows you how to create custom
checks tailored to your application.
...
The function receives the trace and returns a boolean. `from_fn` wraps
...
- `@Check.register("kind")` - Registers the check for polymorphic
  serialization
