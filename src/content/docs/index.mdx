...
- Automated detection of security vulnerabilities using `vulnerability_scan`, which generates hostile inputs and reports the ones your agent handled badly.
- Automated detection of RAG quality failures using `quality_scan`.
- Beyond Giskard's own generators, the scan can run garak's probes and deepteam's attacks against the same agent through the optional `giskard-scan[garak]` and `giskard-scan[deepteam]` extras.

A scan that finds nothing means the generated scenarios did not break your agent. It is not proof that the agent is safe, and it is not a compliance audit.
