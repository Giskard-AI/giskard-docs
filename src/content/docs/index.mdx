---
title: "Giskard: AI Agent Evaluation & Red Teaming Platform"
...
**Giskard Open Source** is a Python library for LLM testing and evaluation. It is available on [GitHub ↗](https://github.com/Giskard-AI/giskard-oss) and formed the basis for our course on Red Teaming LLM Applications on [Deeplearning.AI ↗](https://www.deeplearning.ai/short-courses/red-teaming-llm-applications/).

The library provides a set of tools for testing and evaluating LLMs, including:

- Behavioral tests written as scenarios and checks that pass or fail under pytest. A scenario is one test case: a message (or short conversation) to send to your agent. A check is a rule its reply has to satisfy.
- Automated detection of security vulnerabilities using `vulnerability_scan`, which generates hostile inputs and reports the inputs your agent answered when it should have refused.
- Automated detection of RAG quality failures using `quality_scan`.
- Beyond Giskard's own generators, the scan can run garak's probes and deepteam's attacks against the same agent through the optional `giskard[garak]` and `giskard[deepteam]` extras.

The scan is not installed by default: add the `scan` extra (`pip install --pre "giskard[scan]"`). A scan that finds nothing means the generated scenarios did not break your agent. It is not a safety proof or a compliance audit.
