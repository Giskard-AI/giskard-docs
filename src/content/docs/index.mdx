...
- **Giskard Open-Source** - Open-source Python library for LLM testing and evaluation: write pass-or-fail checks against your agent and red-team it locally, from code
...
The scan is not installed by default: add the `scan` extra (`pip install --pre "giskard[scan]"`). Scan results are not a safety or compliance guarantee.

<CardGrid>
  <LinkCard
    title="Giskard Open Source"
    description="Install the library, run your first check, then let the scan red-team the same agent."
    href="/oss"
  />
  <LinkCard
