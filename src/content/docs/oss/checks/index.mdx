---
title: What are Giskard Checks?
description: "Lightweight Python package for testing non-deterministic AI applications with built-in checks, async-first design, and LLM-as-a-judge evaluations."
sidebar:
  order: 1
---

{/* If you are an AI agent, all the links listed here are available appending .md to the end of the URL */}
{/* For example, the link to the Installation page is https://docs.giskard.ai/oss/checks/installation.md */}

import { LinkCard, CardGrid, Steps } from "@astrojs/starlight/components";

Giskard Checks is a lightweight Python library for testing and evaluating non-deterministic applications such as LLM-based systems.

## What are Giskard Checks

**Giskard Checks** provides a flexible and powerful framework for testing AI applications including RAG systems, agents, summarization models, and more. Whether you're building chatbots, question-answering systems, or complex multi-step workflows, Giskard Checks helps you ensure quality and reliability.

### Key Features

- **Built-in Check Library**: Ready-to-use checks including LLM-as-a-judge evaluations, string matching, equality assertions, and more
- **Flexible Testing Framework**: Support for both single-turn and multi-turn scenarios with stateful trace management
- **Type-Safe & Modern**: Built on Pydantic for full type safety and validation
- **Async-First**: Native async/await support for efficient concurrent testing
- **Highly Customizable**: Easy extension points for custom checks and interaction patterns
- **Serializable Results**: Immutable, JSON-serializable results for easy storage and analysis

Looking for tests you don't have to write? [Giskard Scan](/oss/scan) generates an adversarial suite from a plain-language description of your agent, and returns it as an ordinary `Suite` you run with everything below.

:::tip[Use a coding agent?]
Install the [Giskard Agent Skills](/oss/agent-skills) and let your coding agent (Claude Code, Cursor, and more) design red-team suites or RAG evaluation suites directly from a plain-English description of your agent.
:::

## Start here

New here? Follow these five steps in order.

<Steps>

1. **[Install & Configure](/oss/checks/installation)** (~5 min) — Giskard Checks installed and an LLM provider configured. In a hurry? [Quickstart](/oss/checks/quickstart) shows one end-to-end example.
2. **[Your First Test](/oss/checks/tutorials/your-first-test)** (~10 min) — a passing scenario, with no API key or LLM required.
3. **[Your First LLM Call](/oss/checks/tutorials/single-turn)** (~15 min) — a real model call evaluated with `LLMJudge`.
4. **[Test Suites](/oss/checks/tutorials/test-suites)** (~20 min) — many scenarios grouped into one suite you run with a single await.
5. **[CI/CD Integration](/oss/checks/how-to/ci-cd)** (~20 min) — your suite running automatically on every pull request.

</Steps>

## Browse the docs

<CardGrid>
  <LinkCard
    title="Installation"
    description="Install the library and configure your LLM provider"
    href="/oss/checks/installation"
  />
  <LinkCard
    title="Quickstart"
    description="Your first scenario in under 5 minutes"
    href="/oss/checks/quickstart"
  />
  <LinkCard
    title="Agent Skills"
    description="Drop-in skills that turn Claude Code and other coding agents into Giskard specialists"
    href="/oss/agent-skills"
  />
  <LinkCard
    title="Tutorials"
    description="Step-by-step learning arc from your first test to reusable test suites"
    href="/oss/checks/tutorials"
  />
  <LinkCard
    title="How-to Guides"
    description="Task-oriented guides for pytest, custom checks, CI/CD, spy, and more"
    href="/oss/checks/how-to"
  />
  <LinkCard
    title="API Reference"
    description="Complete API documentation for all checks, scenarios, and utilities"
    href="/oss/checks/reference"
  />
  <LinkCard
    title="Explanation"
    description="Core concepts, async design, JSONPath, and when to use which check"
    href="/oss/checks/explanation"
  />
  <LinkCard
    title="Use Cases"
    description="Worked examples for RAG systems, agents, chatbots, and content moderation"
    href="/oss/checks/use-cases"
  />
</CardGrid>

## Use Cases

Giskard Checks is designed for:

- **RAG Evaluation**: Test groundedness, relevance, and context usage in retrieval-augmented generation systems
- **Agent Testing**: Validate multi-step agent workflows with tool calls and complex reasoning
- **Quality Assurance**: Ensure consistent output quality across model updates and deployments
- **LLM Guardrails**: Implement safety checks, content moderation, and compliance validation
- **Regression Testing**: Track model behavior changes over time with reproducible test suites
