...
<MethodCard name="from_fn()" returnType="Check">
  Build an `FnCheck` from a callable. Every option other than `fn` is keyword-only.
...
The returned object is an `FnCheck`, so the serialization caveat above applies to it too.
