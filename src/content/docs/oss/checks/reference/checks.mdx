...
:::note[The `MISSING` sentinel]
Optional parameters that may legitimately be set to `None` default to the `MISSING`
sentinel rather than `None`. This lets a check distinguish "not provided" from
"explicitly provided as `None`". You never need to pass `MISSING` yourself — just
omit the parameter.
:::
...
<Property name="keyword_key" type="str | MISSING" defaultValue="MISSING">
...
<Property name="pattern_key" type="str | MISSING" defaultValue="MISSING">
...
>
  ...
  When omitted, the resolved value is compared directly. `"any"` passes if at
  least one item matches, `"all"` if every item matches, `"none"` if no item
  matches. Requires a list, set, or tuple.
</Property>
...
check = LessThan(expected_value=500, key="trace.last.metadata.latency_ms")
...
<ResourceSection description="Check that a trace value is valid JSON. Optionally validates against a JSON Schema.">
...
<Property name="parse" type="bool" defaultValue="True">
  When `True` (default), the extracted value must be a serialized JSON string,
  parsed with `json.loads` before validation. When `False`, the value is treated
  as an already-parsed JSON value (dict, list, string, number, boolean, or
  `None`) and only checked for JSON serializability and schema conformance.
</Property>
<Property
...
# Value is already a parsed object, not a JSON string
check = JsonValid(key="trace.last.outputs", parse=False)
...
<ResourceSection description="Check that validates the readability of output text. Extracts a string from the trace, computes a readability score with textstat, and optionally validates that score against configured minimum and maximum thresholds.">
...
<Property name="key" type="str" defaultValue='"trace.last.outputs"'>
  JSONPath expression to extract the text to score.
</Property>
<Property
  name="metric"
  type='Literal["flesch_reading_ease", "flesch_kincaid_grade", "gunning_fog", "automated_readability_index", "coleman_liau_index", "dale_chall_readability_score"]'
  defaultValue='"flesch_reading_ease"'
>
  Readability metric to compute.
</Property>
<Property name="min_score" type="float | None" defaultValue="None">
  Optional lower bound on the score.
</Property>
<Property name="max_score" type="float | None" defaultValue="None">
  Optional upper bound on the score.
</Property>

```python
from giskard.checks import Readability

check = Readability(min_score=60.0)
...
<TypeTable name="LLMCheckResult">
  ...
  <Property name="reason" type="str" required>
    Explanation for the result. Must be a non-empty string.
  </Property>
</TypeTable>
...
<ResourceSection description="LLM-based check that fails only on clear contradictions with context. Uses the same inputs as Groundedness but applies a permissive criterion: omissions and unsupported additions are tolerated unless they directly conflict with the reference context.">
...
<Property
  ...
  defaultValue="MISSING"
>
  Reference context document(s) the answer must not contradict (static).
</Property>
<Property
  name="context_key"
  type="str"
...
:::tip[Contradiction vs Groundedness]
`Groundedness` fails when the answer is not fully supported by the context.
`Contradiction` only fails when the answer directly conflicts with it — use it
when extra or omitted detail is acceptable.
:::
...
<Property name="include_history" type="bool" defaultValue="True">
  Whether to include the prior conversation history in the evaluation prompt.
</Property>
<Property
  name="generator"
  type="BaseGenerator"
...
<Property
  ...
  defaultValue="get_default_embedding_model()"
>
