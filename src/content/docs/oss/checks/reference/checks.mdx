...
:::caution
The path field is `target_key` on every built-in check. Passing `key`, `text_key`, or
`answer_key` raises a `ValidationError` — the models reject unknown keyword arguments.
:::
