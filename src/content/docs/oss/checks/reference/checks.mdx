...
<ResourceSection description="Evaluate an inline Rego policy against trace data using regorus (Open Policy Agent). Extracts the OPA input document from the trace via JSONPath, merges optional static data, and evaluates a boolean rule.">

**Module:** `giskard.checks.builtin.rego_policy`

:::note[Optional dependency]
Requires the `regorus` extra: `pip install 'giskard-checks[regorus]'` (installs the `celine-regorus` wheel; import as `regorus`).
:::
