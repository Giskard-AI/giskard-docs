...
Examples on this page use a customer-support agent for a retail bank, the same agent used across the checks reference.
...
scenario = (
    ...
    .interact(
        inputs=lambda trace: f"You said: {trace.last.outputs} Is it the same at a weekend?",
        outputs=lambda inputs: bank_support_agent(inputs),
    )
)
```
...
result = await (
    Scenario("bank_agent_dispute_advice")
    .interact(
        inputs="There is a payment on my card I don't recognise.",
        outputs="I can help with that. Can you tell me the date and amount?",
    )
    .check(
        FnCheck(
    ...
    .interact(
        inputs="89.00 EUR on 4 March, to Rail Europe.",
        outputs="I've raised a dispute. Investigations take up to 15 working days.",
    )
    .check(
        StringMatching(
            keyword="15 working days", target_key="trace.last.outputs"
        )
    )
    .run()
)
...
scenario = Scenario(
    name="bank_agent_greeting",
    steps=[
        Step(
            interacts=[
                Interact(
                    inputs="Hi, can you help me with a card problem?",
                    outputs="Of course. Is the card lost, blocked, or showing a payment you don't recognise?",
                )
            ],
            checks=[
