...
Examples on this page use BotaniBot, an assistant for a garden centre.
...
interaction = Interaction(
    inputs="How often should I water a Monstera deliciosa?",
    outputs="Let the top 2 cm of soil dry out before watering again.",
    metadata={"model": "gpt-5", "tokens": 15},
)

# Structured, so checks can address individual fields
interaction = Interaction(
    inputs={"question": "Do you stock Monstera deliciosa?", "store": "ghent"},
    outputs={"availability": "in_stock", "price_eur": 24.99},
    metadata={"retrieval_score": 0.91, "latency_ms": 320},
)
```

Structured outputs are worth the effort: they let rule-based checks assert on a field instead of parsing a sentence.
...
scenario = (
    ...
    .interact(
        inputs=lambda trace: f"You said: {trace.last.outputs} Does that change in winter?",
        outputs=lambda inputs: botanibot(inputs),
    )
)
```
...
result = await (
    Scenario("botanibot_repotting_advice")
    .interact(
        inputs="I bought a Monstera from you last week.",
        outputs="Lovely choice. How can I help with it?",
    )
    .check(
        FnCheck(
            fn=lambda trace: "?" in trace.last.outputs,
            name="asks_a_follow_up",
        )
    )
    .interact(
        inputs="Should I repot it now?",
        outputs="Wait until roots show through the drainage holes, usually in spring.",
    )
    .check(
        StringMatching(keyword="spring", text_key="trace.last.outputs")
    )
    .run()
)
...
availability = resolve(trace, "trace.last.outputs.availability")  # "in_stock"

missing = resolve(trace, "trace.last.outputs.pot_size_cm")
