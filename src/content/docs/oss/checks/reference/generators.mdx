...
<!-- pyright-skip: This usage fragment depends on the user simulator configured in the surrounding application. -->
```python
...
test_scenario = (
    ...
    .check(
        FnCheck(
            fn=lambda trace: "payment" in trace.last.outputs.lower(),
...
<!-- pyright-skip: This continuation uses the scenario created in the preceding example. -->
```python
result = await test_scenario.run()
