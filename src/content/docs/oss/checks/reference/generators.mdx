...
<ResourceSection description="Yield a fixed dataset prompt as the next target input.">
...
<Property name="generator" type="BaseGenerator | None" defaultValue="get_default_generator()">
  LLM used only when the target input type is structured (not `str`). Unused when the input type is `str`.
</Property>
...
scenario = Scenario("dataset_row").interact(inputs=gen, outputs=target)
```
