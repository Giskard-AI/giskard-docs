...
Groups interaction specs and checks executed together before advancing the shared trace. The fluent `Scenario.interact()` / `.check()` API builds steps implicitly; use `Step` when constructing scenarios from pre-built components.

<TypeTable name="Step">
  <Property name="interacts" type="list[InteractionSpec]">
    Interaction specs run in this step.
  </Property>
  <Property name="checks" type="list[Check]">
    Checks validated after this step's interactions.
  </Property>
</TypeTable>
...
<MethodCard name="generate_suite()" returnType="Suite">
  <Property name="description" type="str" required>
    Description of the agent under test. Injected into each scenario's annotations as `"description"`.
  </Property>
  <Property name="categories" type="list[ScenarioCategory] | None" defaultValue="None">
  ...
  <Property name="name" type="str" defaultValue='"Security Suite"'>
    Suite name.
  </Property>
</MethodCard>
