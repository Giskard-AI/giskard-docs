...
:::note[Internal machinery]
`ValueProvider` and `ValueGenerator` are used internally by the framework to normalize how checks and interaction specs source their inputs. Most users never construct them directly — they are documented here so the behaviour they impose on _your_ callables, such as which parameter names may be injected, is written down somewhere.
:::
...
Providers are awaited, so the calls below are shown inside a coroutine. In a notebook you can `await` them directly instead.
...
async def main():
    ...
    print(await static_provider())  # "hello"

    # Callable with injected kwargs
    callable_provider = ValueProvider(
        value_or_callable=greet, kwargs_keys={"name"}
    )
    print(await callable_provider(name="world"))  # "Hello, world!"
...
asyncio.run(normalize_to_async())
```
