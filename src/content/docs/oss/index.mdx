...
The v2 **Scan** is now `giskard.scan.vulnerability_scan`, and **RAGET** is replaced by `giskard.scan.quality_scan` with a `KnowledgeBase`. Both ship in the `scan` extra: `pip install --pre "giskard[scan]"`. See the [CHANGELOG ↗](https://github.com/Giskard-AI/giskard-oss/blob/main/CHANGELOG.md) for the full breaking-change list, or the [v3 announcement ↗](https://github.com/orgs/Giskard-AI/discussions/2250).
:::
