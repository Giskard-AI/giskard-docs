...
| `giskard.llm.set_llm_model("gpt-4o")`                   | `set_default_generator(Generator(model="openai/gpt-4o-mini"))`                    |
...
- **Every entry point is a coroutine.** `vulnerability_scan`, `quality_scan`, `generate_suite`, and `Suite.run` must be awaited. The v3 snippets on this page use top-level `await`, which works in a notebook; in a plain script, put them in an `async def main()` and call `asyncio.run(main())`.
- **The automatic tabular scan is gone.** There is no successor to `model_type="classification"` or `model_type="regression"`, and nothing introspects a model and dataset to find performance, bias, and robustness issues on its own. You can still wrap an ML model as a target and write checks against it, but if you relied on the automatic tabular scan, stay on v2.
...
- There is **no HTML report**. `print_report()` writes to the terminal, `to_junit_xml()` feeds CI, and `model_dump_json()` gives you the raw result to store. Nothing renders a standalone page. Both scans already print a grouped report before returning, so the explicit `print_report()` above is only useful when you want a second view under a different `group_by`.
...
| Information Disclosure           | no tag of its own: disclosure attempts arrive as `threat-type:prompt-injection`                  |
...
# The provider prefix picks the SDK: openai/, google/, gemini/, anthropic/, azure/, azure_ai/
set_default_generator(Generator(model="openai/gpt-4o-mini"))
```

`Generator` calls each provider's native SDK, so install the matching extra: `pip install --pre "giskard[openai]"`, `[google]`, `[anthropic]`, `[azure]`, or `[all-llms]`. `Generator` is the public name; `GiskardLLMGenerator` is the same class under its internal name.

For a provider without a native integration, ask for LiteLLM explicitly after `pip install --pre "giskard[litellm]"`:
...
API keys are still read from the provider's own environment variable: `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `GEMINI_API_KEY` (or `GOOGLE_API_KEY`), `AZURE_API_KEY`, `AZURE_AI_API_KEY`. Full detail in [Install & Configure](/oss/scan/installation) and the [settings reference](/oss/checks/reference/settings).
...
There is no replacement for the performance, drift, metamorphic, and statistical test catalogs: those were built around the v2 model and dataset wrappers, which v3 does not have.

## What has no v3 equivalent

Each of these silently breaks a v2 script, so check the list before you start rewriting:

- **The automatic tabular scan and the `giskard.testing` ML test suite.** `model_type="classification"`, `classification_labels`, `cat_columns`, and `target=` have no successor, and neither does the detector suite that read a `giskard.Model` plus a `giskard.Dataset`. Wrapping an ML model as a v3 target and writing your own checks against it still works; the automatic detection does not. Stay on `giskard[llm]>2,<3` for that.
