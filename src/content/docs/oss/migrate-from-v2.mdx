...
For a provider without a native integration, ask for LiteLLM explicitly after installing `pip install --pre "giskard[litellm]"`:
...
API keys are still read from the provider's own environment variable: `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `GEMINI_API_KEY`, `GOOGLE_API_KEY`, `AZURE_API_KEY`, or `AZURE_AI_API_KEY`. Full detail in [Install & Configure](/oss/scan/installation) and the [settings reference](/oss/checks/reference/settings).
