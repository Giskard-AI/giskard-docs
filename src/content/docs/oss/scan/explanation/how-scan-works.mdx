...
The examples below run against `support_agent`, the retail-bank support agent wrapped in [Wrap your agent](/oss/scan/how-to/wrap-your-agent). The tutorial uses a garden-center assistant instead, because it runs live against a model:
...
{/* pyright-skip: Alternative tab example; it reuses support_agent from the wrapping guide. */}
```python
...
- [Your First Scan](/oss/scan/tutorials/your-first-scan) to run the pipeline end to end
