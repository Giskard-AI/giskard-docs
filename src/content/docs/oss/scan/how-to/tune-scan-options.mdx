...
The examples below scan a retail bank's support agent, because it has both the refusal rules a vulnerability scan probes and the policy documents a quality scan needs. Define the target and the documents once:
...
```python
options: ScanOptions = {"max_scenarios": 20, "seed": 7, "max_concurrency": 4}
