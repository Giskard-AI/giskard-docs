...
{/* pyright-skip: This integration example imports the reader's bank agent. */}
```python
...
{/* pyright-skip: Alternative tab example; it reuses the reader's bank agent. */}
```python
...
{/* pyright-skip: This fragment reuses options from the reader's configuration. */}
```python
result = await vulnerability_scan(
    target=bank_agent,
    description=DESCRIPTION,
