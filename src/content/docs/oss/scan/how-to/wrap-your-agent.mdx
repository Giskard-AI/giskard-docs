...
Every Scan generator returns a complete instance of the Pydantic input model you declared. Generators can produce that structured message directly; this is how the default adversarial generators and Crescendo work.

Dataset-backed generators and third-party scanners start with textual test prompts instead. For these generators, Giskard maps each prompt into your input schema before calling the target. It places the prompt in the string field that most naturally carries a user message and fills other required fields with short, schema-valid placeholder values.
...
For a dataset-backed or third-party scan, Giskard can place the attack or quality prompt in `body` while supplying valid values for `subject` and `priority`. The mapping is inferred once from the schema; the mapping model does not see the generated test prompt. If the schema has no string field that can carry a user message, Scan raises an error instead of guessing.

### Multi-agent systems

To scan a multi-agent application, wrap the whole application as the target callable. The wrapper can invoke a supervisor, router, or agent graph internally and return the final structured result. Giskard tests that external system boundary; it does not orchestrate or address the internal agents separately.
