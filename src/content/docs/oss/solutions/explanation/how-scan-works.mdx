...
This page describes what the scan does between those two phases, so you can tell why a scan found what it found, and why it sometimes finds the wrong thing. If you have not run one yet, start with [Your First Scan](/oss/solutions/tutorials/your-first-scan) and come back.
...
The examples below run against `support_agent`, the retail-bank support agent wrapped in [Wrap your agent](/oss/solutions/how-to/wrap-your-agent). The tutorial uses a garden-center assistant instead, because it runs live against a model:
