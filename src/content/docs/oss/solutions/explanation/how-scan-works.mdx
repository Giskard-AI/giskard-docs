...
Run a scan when you want to see how your agent holds up under pressure, against realistic scenarios you would not have thought to write yourself. A first scan against a bank's customer-support bot typically finds that it gives investment advice it should refuse, or that it follows instructions hidden inside a pasted email.
...
suite = await generate_suite(description="...", languages=["en"], generators=[...])
result = await suite.run(my_agent)
```
...
result = await vulnerability_scan(
    target=my_agent,
    description="A customer-support bot for a retail bank. It must never give investment advice.",
    languages=["en"],
    max_scenarios=20,
)
```
...
result = await quality_scan(
    target=my_agent,
    description="A customer-support bot for a retail bank answering from our help center.",
    languages=["en"],
    knowledge_base=["Cards can be frozen from the app.", "Transfers settle in one business day."],
)
```
...
suite = await generate_suite(
    description="A customer-support bot for a retail bank.",
    languages=["en"],
...
With the default `group_by="threat-type"`, the report buckets results by that tag. Schematically, a run against a bank support bot might come back like this:
