...
This page describes what the scan does between those two phases, so you can tell why a scan found what it found, and why it sometimes finds the wrong thing. If you have not run one yet, start with [Scan Vulnerabilities](/oss/solutions/scan-vulnerabilities) and come back.
...
The examples below run against `support_agent`, the retail-bank support agent wrapped in [Wrap your agent](/oss/solutions/how-to/wrap-your-agent):
...
- [Scan Vulnerabilities](/oss/solutions/scan-vulnerabilities) to run the pipeline end to end
