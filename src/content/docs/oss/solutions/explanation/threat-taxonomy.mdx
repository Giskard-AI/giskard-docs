...
Two more prefixes show up on individual scenarios. `owasp:` maps a scenario to an OWASP LLM Top-10 entry, and `probe-type:` records the attack shape (`multi-turn`, meaning the attack is spread over several messages rather than one). Both are groupable, neither is a default.
...
[Prompt injection](/start/glossary/security/injection) is instructions smuggled into content the agent processes (pasted text, retrieved documents, tool output) that the agent then obeys. `PromptInjectionScenarioGenerator` emits it, paired with `owasp:llm-top-10-2025:LLM01`.
