...
suite_result = await suite.run(target=my_agent, parallel=True)
```

`Suite.run` defaults to `parallel=False`, unlike the scan helpers. Pass `parallel=True` when you want the same concurrency a full scan gives you.
...
The budget is split by a multinomial draw, so a generator that draws zero is skipped. See [the scenario budget](/oss/solutions/explanation/how-scan-works#the-scenario-budget).
...
Some generators are backed by datasets whose licenses do not permit commercial use. Drop them with `commercial_use=True`:
...
Prompt it with something like _"red-team my support bot for PII leaks and competitor mentions"_. The full set of skills is at [Giskard Skills ↗](https://github.com/Giskard-AI/giskard-skills).
