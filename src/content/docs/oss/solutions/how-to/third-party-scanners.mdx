...
Treat `list_scan_items` as the authority rather than the upstream READMEs. For garak it enumerates the probes of the garak version you actually installed, so it never drifts. For deepteam it matters more: Giskard supports a fixed subset of deepteam's catalog, and names outside that subset are silently skipped instead of raising.
...
Read [NVIDIA/garak](https://github.com/NVIDIA/garak) ↗ and [confident-ai/deepteam](https://github.com/confident-ai/deepteam) ↗ for background on what each probe or technique actually does.
...
Await it, as you await `vulnerability_scan`. Every scan entry point in the library is `async def`.

The target is `botanibot`, the garden-center assistant from [Your First Scan](/oss/solutions/tutorials/your-first-scan):
...
`AutoDANCached` replays pre-computed DAN-style jailbreak prefixes; `SlurUsage` checks whether the agent will produce ethnic slurs. Both are worth running against any customer-facing agent, whatever it sells, because neither depends on what the agent is for. That is also the reason to pick garak: it brings attacks nobody on your team would have written. Two named probes is the shape of a first run. Move to `probes=None` for the curated six once you know the wiring works, and treat `"all"` as a deliberate, expensive decision.
...
A garden center has little worth leaking, so the example switches to a retail-bank support agent, which holds account data and a system prompt worth extracting. It is wrapped exactly like BotaniBot, with the customer records in its context:
...
async def bank_support_bot(inputs: AgentInput) -> AgentOutput:
