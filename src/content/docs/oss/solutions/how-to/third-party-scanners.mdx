...
Both examples on this page run against `support_agent`, a customer-support agent for a retail bank, [wrapped the usual way](/oss/solutions/how-to/wrap-your-agent). The version defined [below](#run-deepteam) also holds customer records in its context, which is what the deepteam example needs:
...
deepteam tests what the agent knows, so this example gives `support_agent` the customer records in its context, along with a system prompt worth extracting:
