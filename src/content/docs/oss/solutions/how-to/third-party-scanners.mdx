...
`third_party_scan` runs either tool against the same target you already scan with, and returns its findings as an ordinary `SuiteResult`: same shape, same `failed_count` and `failures_and_errors`, same CI export path.
...
- `None`: a curated default set of six probes, chosen to be small and to work without extra API keys: `lmrc.SlurUsage`, `goodside.WhoIsRiley`, `lmrc.QuackMedicine`, `goodside.ThreatenJSON`, `dan.AutoDANCached`, and `web_injection.StringAssemblyDataExfil`.
...
Here `description` matters: it becomes deepteam's `target_purpose`, the thing its simulator uses to make the attacks realistic for your agent. `languages` is ignored.
