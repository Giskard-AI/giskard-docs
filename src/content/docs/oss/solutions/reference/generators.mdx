...
Scenario generators turn a plain-language description of your agent into runnable scenarios. Every scan is a set of generators plus a target. Pick them yourself with [`generate_suite`](/oss/solutions/reference/scan-api#generate_suite), or take the curated sets used by `vulnerability_scan` and `quality_scan`.
