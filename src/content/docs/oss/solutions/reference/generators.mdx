...
suite = await generate_suite(
    description=(
        ...
        "plants, soil and watering from our care guides. It must never give "
        "medical advice about ingesting plants."
    ),
    languages=["en", "fr"],
    generators=[
...
This mix crosses the two families on purpose: the first two attack the agent, and `SycophancyScenarioGenerator` catches the opposite failure, where a confident customer pushes the agent off what the care guides actually say. Mixing families needs `knowledge_base`, because the knowledge-base generators produce nothing without it and would silently contribute zero scenarios.
