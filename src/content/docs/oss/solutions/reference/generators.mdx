...
Which generators you run decides what the scan can find. Drop one and the failures it would have found never appear in the report. The table below is the whole catalog, so it is also the limit of what a scan covers.
