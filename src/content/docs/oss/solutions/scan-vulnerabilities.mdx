...
:::tip[New to the scan?]
Start with [Your First Scan](/oss/solutions/tutorials/your-first-scan) — a guided run against a toy agent in a few minutes. This page is the complete guide for wiring the scan to *your* agent.
:::
...
The families below are the ones you will see most often in a report, mapped to the [OWASP LLM Top 10](https://genai.owasp.org/llm-top-10/) and the vulnerability categories used across Giskard:
...
They are not the whole catalog — a vulnerability scan also runs Crescendo multi-turn attacks, GCG suffix injections, and two Hugging Face attack datasets; the [generators reference](/oss/solutions/reference/generators) is the source of truth. The library keeps adding more. See the [roadmap](#roadmap) for what is coming, the [vulnerability categories catalog](/hub/ui/scan/vulnerability-categories) for the complete list of categories, and [How the Scan Works](/oss/solutions/explanation/how-scan-works) for how generators, threat types, and target modes fit together.
...
First, install the scan and choose the model that will generate and judge the scenarios. `giskard-scan` pulls in the rest of the library, so you only add the `giskard-agents` extra for the provider you use. Pick yours below — [Install & Configure](/oss/solutions/installation) has the longer version.
...
```bash
pip install --pre giskard-scan "giskard-agents[anthropic]"
```
...
```bash
pip install --pre giskard-scan "giskard-agents[google]"
```
...
```bash
pip install --pre giskard-scan "giskard-agents[litellm]"
```
...
### Save the suite and run it in CI

Generating scenarios uses an LLM, so generate the suite once and replay it. `suite_result.suite` is a Pydantic model you can serialize to JSON, commit, and load back in your pipeline — no regeneration, just judging:
...
The full workflow, including the JUnit XML export and a GitHub Actions job, is in [Run the Scan in CI](/oss/solutions/how-to/scan-in-ci).

### Understand what the scan did

[How the Scan Works](/oss/solutions/explanation/how-scan-works) explains the generators, target modes, threat-type grouping, and why judge verdicts should be read rather than trusted outright.

### Read the API reference

Every argument, generator, and type used on this page is documented in the scan reference:

- [Scan API](/oss/solutions/reference/scan-api) -- `vulnerability_scan`, `quality_scan`, `generate_suite`, `third_party_scan`, `list_scan_items`, `ScanOptions`, `SuiteGeneratorRegistry`
- [Generators](/oss/solutions/reference/generators) -- The full scenario generator catalog
- [Knowledge Base](/oss/solutions/reference/knowledge-base) -- `KnowledgeBase` and `Document` for the quality scan

## Advanced usage

You can customize the scan by passing options directly to `vulnerability_scan`. See the [Scan API reference](/oss/solutions/reference/scan-api) for the complete list.
