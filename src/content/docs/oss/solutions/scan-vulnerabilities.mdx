...
Every scan entry point is a coroutine. The `await` above works as-is in a notebook; in a `.py` script, put it inside an `async def main()` and call `asyncio.run(main())`.
...
Then export the result for your CI reporter. `to_junit_xml()` always returns the XML as a string; pass `path` to write a file as well (parent directories are created). The returned string has no XML declaration; the written file does.
